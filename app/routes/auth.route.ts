...
router.post(subRoutes.login, async (req: Request, res: Response) => {
 const response = await authService.login(res.locals.ctx.dbProviders.postgreSqlProvider, {
  username: req.body.username,
  password: req.body.password
 });
...
router.post(subRoutes.logout, async (req: Request, res: Response) => {
 await authService.logout(res.locals.ctx.dbProviders.postgreSqlProvider, req.body.token);
 res.status(204).json({});
});

router.post(subRoutes.token, async (req: Request, res: Response) => {
 const accessToken = await authService.token(res.locals.ctx.dbProviders.postgreSqlProvider, req.body.token);
...
router.post(subRoutes.verify, async (req: Request, res: Response) => {
 await authService.verify(res.locals.ctx.dbProviders.postgreSqlProvider, req.query.token);
 res.status(200).json({});
});

router.post(subRoutes.forgetPassword, async (req: Request, res: Response) => {
 await authService.forgetPassword(res.locals.ctx.dbProviders.postgreSqlProvider, req.body.username);
 res.status(200).json({});
});

router.post(subRoutes.resetPassword, async (req: Request, res: Response) => {
 await authService.resetPassword(res.locals.ctx.dbProviders.postgreSqlProvider, {
  username: req.body.username,
  password: req.body.password
 }, req.body.token);
 res.status(200).json({});
