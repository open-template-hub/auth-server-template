...
export class TokenRepository {
 ...
 insertToken = async (token) => {
  let res;
  try {
   res = await this.provider.query('INSERT INTO tokens(token, expire_date) VALUES($1, $2)',
 ...
 deleteToken = async (token) => {
  let res;
  try {
   res = await this.provider.query('DELETE FROM tokens WHERE token LIKE $1',
    [token]);
  } catch (error) {
   console.error(error);
   throw error;
 ...
 findToken = async (token) => {
  let res;
  try {
   res = await this.provider.query('SELECT token FROM tokens WHERE token LIKE $1',
    [token]);
  } catch (error) {
   console.error(error);
   throw error;
 ...
 deleteExpiredTokens = async () => {
  let res;
  try {
   res = await this.provider.query('DELETE FROM tokens WHERE expire_date < (now() at time zone \'utc\')', []);
  } catch (error) {
   console.error(error);
   throw error;
  }
