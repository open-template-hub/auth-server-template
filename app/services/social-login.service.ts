...
export class SocialLoginService {
 loginUrl = async (db, data) => {
  ...
  let socialLoginParams = await socialLoginRepository.findSocialLoginByKey(data.key);
 ...
 login = async (db, data) => {
  ...
  const socialLoginRepository = new SocialLoginRepository(db);
  const socialLoginParams = await socialLoginRepository.findSocialLoginByKey(data.key);
 ...
 loginUserWithUserData = async (db, key, userData) => {
  ...
  const socialLoginRepository = new SocialLoginRepository(db);
  let socialLoginUser = await socialLoginRepository.findMappingDataByExternalUserId(key, userData.external_user_id);
  ...
  if (socialLoginUser) {
   ...
   await socialLoginRepository.insertSocialLoginMapping(key, userData.external_user_id, userData.external_username, userData.external_user_email, autoGeneratedUserName);
 ...
 signup = async (db, user) => {
  ...
  await userRepository.insertUser({username: user.username, password: hashedPassword});
